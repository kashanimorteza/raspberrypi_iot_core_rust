...
pub async fn add_sample_configs(db: &DatabaseConnection) -> Result<(), Box<dyn std::error::Error>> 
{
    ...
    let sample_configs = vec![
        ConfigActiveModel 
        {
            ...
            path_api: Set("/root/example_raspberrypi_api".to_string()),
            path_gui: Set("/root/example_raspberrypi_gui".to_string()),
            webapi_title: Set("microcontroller".to_string()),
            webapi_description: Set("This is my custom API schema description".to_string()),
