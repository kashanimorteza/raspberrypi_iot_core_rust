...
#[utoipa::path(
    get,
    path = "/timer_item/status/{id}",
    tag = "⏰📝 Timer Item",

    params(
        ("id" = i32, Path, description = "Timer Item ID to toggle status")
    ),
    responses(
        (status = 200, description = "Timer Item status toggled successfully", body = TimerItemModel),
        (status = 404, description = "Timer Item not found"),
        (status = 500, description = "Internal server error")
    )
)]
pub async fn status_timer_item(
    State(state): State<AppState>,
    Path(id): Path<i32>,
) -> Result<Json<ModelOutput<TimerItemModel>>, StatusCode> {
    let service = TimerItemService::new();
