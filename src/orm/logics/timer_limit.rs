...
impl TimerLimitORM
{
    ...
    pub async fn status(&self, db: &DbConn, id: i32) -> ModelOutput<TimerLimitModel>
    {
        ...
        match TimerLimitEntity::find_by_id(id).one(db).await
        {
                ...
                match active.update(db).await
                {
                    Ok(updated) => {
                        let message = if current_enable {
                            "Timer limit disabled successfully".to_string()
                        } else {
                            "Timer limit enabled successfully".to_string()
                        };
            ...
            Ok(None) => ModelOutput::error("Timer limit not found".to_string()),
            Err(e) => {
                let error_msg = format!("Database error in {}::{}: {}", self.this_class, this_method, e);
                error!("{}::{} - Error: {}", self.this_class, this_method, error_msg);
                ModelOutput::error(error_msg)
